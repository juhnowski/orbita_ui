...
+------+-----+---------+--------+---------+
|Канал |Включ|IP и порт|Наруж.  |Настройка|
+------+-----+---------+--------+---------+
|      |     |         |        |         |
+------+-----+---------+--------+---------+
...
function orbita_camera.init()
	s = box.space.camera;
	if not s then
		s = box.schema.space.create('camera');
...
function orbita_camera.add(addr, port, name_ch, num, nameProtocol)
	local i = box.space.camera:len();
	i = i + 1;
	box.space.camera:insert{i,addr, port, name_ch, num, nameProtocol};
end

function orbita_camera.del(ind)
	local i = box.space.camera:len();
	if i ~= 0 then
		box.space.camera:delete(ind);
	end
end

function orbita_camera.get(ind)
	return box.space.camera:select{ind};
end

function orbita_camera.getLen()
	return box.space.camera:len();
end

function orbita_camera.clear()
	box.space.camera:truncate();
end
